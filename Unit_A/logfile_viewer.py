...
if __name__ == '__main__':
    ...
    frame1.pack()
    world_canvas = tk.Canvas(frame1,width=canvas_extents[0],height=canvas_extents[1],bg="white")
    world_canvas.pack(side=tk.LEFT)
    sensor_canvas = tk.Canvas(frame1,width=sensor_canvas_extents[0],height=sensor_canvas_extents[1],bg="white")
    sensor_canvas.pack(side=tk.RIGHT)
    scale = tk.Scale(root, orient=tk.HORIZONTAL, command = slider_moved)
    scale.pack(fill=tk.X)
    info = tk.Label(root)
    info.pack()
    frame2 = tk.Frame(root)
    frame2.pack()
    add_file_button = tk.Button(frame2, text="Load (additional) logfile", command=add_file)
    add_file_button.pack(side=tk.LEFT)
    reload_all = tk.Button(frame2,text="Reload all",command=load_data)
    reload_all.pack(side=tk.RIGHT)
